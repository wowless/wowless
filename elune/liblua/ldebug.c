      ...
      case iAsBx: {
        ...
        if (getBMode(op) == OpArgR) {
          ...
          if (dest > 0) {
            ...
            /* if 'j' is even, previous value is not a setlist (even if it looks
             * like one) */
            check((j & 1) == 0);
          }
        }
...
static const char *getfuncname(lua_State *L, CallInfo *ci, const char **name) {
  Instruction i;
  if ((isLua(ci) && ci->tailcalls > 0) || L->ci == L->base_ci ||
      !isLua(ci - 1)) {
...
lua_Clock luaG_clockrate(const global_State *g) { return g->tickfreq; }
