...
describe('frame', function()
  ...
  it('maintains kid order', function()
    ...
    local g = api.env.CreateFrame('frame', nil, f)
    assert.same(1, f:GetNumChildren())
    do (function(...)
      assert.same(1, select('#', ...))
      assert.equals(g, select(1, ...))
    end)(f:GetChildren()) end
    local h = api.env.CreateFrame('frame', nil, f)
    ...
    g:SetParent(api.env.CreateFrame('frame'))
    assert.same(1, f:GetNumChildren())
    do (function(...)
      assert.same(1, select('#', ...))
...
--[[
  ...
  it('has a metatable containing only a mutable __index table', function()
    local f = api.env.CreateFrame('frame', 'moo')
  ...
  it('support $parent in frame names', function()
    ...
    local mooCow = api.env.CreateFrame('frame', '$parentCow', api.env.Moo)
    local topCow = api.env.CreateFrame('frame', '$parentCow')
