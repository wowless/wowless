...
describe('frame', function()
  it('has expected defaults', function()
    local f = api.env.get('CreateFrame')('frame')
    local expected = {
      height = 0,
      numPoints = 0,
      object = { [0] = f[0] },
  ...
  it('is added to global environment if named', function()
    local f = api.env.get('CreateFrame')('frame', 'sillyName')
    assert.equals(f, api.env.get('sillyName'))
  end)

  it('maintains kid order', function()
    local f = api.env.get('CreateFrame')('frame')
    assert.same(0, f:GetNumChildren())
    local g = api.env.get('CreateFrame')('frame', nil, f)
    ...
    local h = api.env.get('CreateFrame')('frame', nil, f)
    ...
    g:SetParent(api.env.get('CreateFrame')('frame'))
    assert.same(1, f:GetNumChildren())
    do
      (function(...)
        assert.same(1, select('#', ...))
  ...
  it('handles show/hide across three generations', function()
    local a = api.env.get('CreateFrame')('frame')
    local b = api.env.get('CreateFrame')('frame', nil, a)
    local c = api.env.get('CreateFrame')('frame', nil, b)
  ...
  it('shares metatables across instances', function()
    local f = api.env.get('CreateFrame')('frame')
    local g = api.env.get('CreateFrame')('frame')
  ...
  it('gets a new method when hooked', function()
    local f = api.env.get('CreateFrame')('frame')
    api.env.get('hooksecurefunc')(f, 'Show', function() end)
  ...
  it('has a metatable containing only a mutable __index table', function()
    local f = api.env.get('CreateFrame')('frame', 'moo')
  ...
  it('support $parent in frame names', function()
    api.env.get('CreateFrame')('frame', 'Moo')
    local mooCow = api.env.get('CreateFrame')('frame', '$parentCow', api.env.get('Moo'))
    local topCow = api.env.get('CreateFrame')('frame', '$parentCow')
    assert.equals(mooCow, api.env.get('MooCow'))
    assert.same('MooCow', mooCow:GetName())
    assert.equals(topCow, api.env.get('TopCow'))
