...
LUA_API int lua_getexceptmask(lua_State *L) { return cast_int(L->exceptmask); }
...
LUA_API int lua_ishookallowed(lua_State *L) { return cast_int(L->allowhook); }
...
LUA_API void lua_setcompatopt(lua_State *L, int opt, int val) {
  switch (opt) {
    ...
    case LUA_COMPATINERRORHANDLER:
      L->compatmask =
          cast_byte((L->compatmask & ~bitmask(opt)) | ((val & 0x1) << opt));
      return;
    default:
      return;
  }
