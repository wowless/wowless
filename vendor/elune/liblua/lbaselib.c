...
static int luaB_pcallwithenv(lua_State *L) {
  ...
  if (!lua_isfunction(L, 1) || !lua_istable(L, 2)) {
    luaL_error(L,
               "Usage: local success, ... = pcallwithenv(f, env, [args...])");
  }
  ...
  if (luaL_getmetafield(L, 2, "__environment")) {
    luaL_error(
        L,
        "cannot use pcallwithenv on a function with a protected environment");
  }
...
static int luaB_hooksecurefunc(lua_State *L) {
  ...
  lua_restoretaint(L,
                   &entryts); /* restore initial entry taint in case of error */
  ...
  lua_setvaluetaint(L, 3,
                    savedts.stacktaint); /* origfunc should have lookup taint */
  lua_setvaluetaint(L, 4,
                    entryts.stacktaint); /* hookfunc should have entry taint */
