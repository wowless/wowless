...
-- luacheck: globals pcallwithenv debuglocals debugstack
-- luacheck: globals variable globalfoo bar CreateFrame
...
case('seterrorhandler: ignores \'*\' source if first local is not a named object', function()
  ...
  assert(
    lasterror == [=[[string "*:OnBanana"]:1: attempt to perform arithmetic on global 'ImmediateError' (a nil value)]=]
  )
end)
