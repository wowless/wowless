...
local function doGetFn(api, loader, apicfg, db2s, sqlite)
  if apicfg.status == 'autogenerated' or apicfg.status == 'unimplemented' then
    return setfenv(getStub(apicfg.outputs or {}), api.env)
  elseif apicfg.status == 'implemented' and data.sql[apicfg.name] then
    local stmt = sqlite:prepare(data.sql[apicfg.name])
    local outs = {}
...
local function getFn(api, loader, apicfg, db2s, sqlite)
  local stub = doGetFn(api, loader, apicfg, db2s, sqlite)
...
local function loadFunctions(api, loader)
  local fns = {}
  local sqlite = require('wowapi.sqlite')(loader.product)
  ...
  for fn, apicfg in pairs(loadApis(loader.product)) do
    ...
    elseif apicfg.stdlib then
      ...
      local bfn = getFn(api, loader, apicfg, db2s, sqlite)
