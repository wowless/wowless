...
local function doGetFn(api, env)
  if api.status == 'autogenerated' or api.status == 'unimplemented' then
    return getStub(api.signatures and api.signatures[1] and api.signatures[1].outputs or {})
...
local function getFn(api, env)
  ...
  local outputs = api.signatures and api.signatures[1] and api.signatures[1].outputs
  return outputs == nil and stub or function(...)
    return (function(...)
...
local function loadFunctions(version, env, log)
  ...
  for fn, api in pairs(loadApis(version)) do
    ...
    local impl = (function()
      local apisig = api.signatures and api.signatures[1]
      if not apisig then
        return bfn
      end
      if apisig.inputs then
        local sig = apisig.inputs
        return function(...)
      ...
      if apisig.oldinputs == nil then
        return bfn
      end
      local apisigs = apisig.oldinputs
